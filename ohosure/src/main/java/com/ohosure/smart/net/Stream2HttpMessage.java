...
public class Stream2HttpMessage {
    ...
    private final static String TAG = Stream2HttpMessage.class.getSimpleName();
    private static final byte CR = '\r';
    private static final byte LF = '\n';
    private static final byte[] CRLF = {CR, LF};

    public int readResponse(InputStream in, HttpMessage mHttpResponseMessage)
            throws IOException, InterruptedException {
        // 读取状态行
        String statusLine = readStatusLine(in);
        if (statusLine == null)
            return -1;
        MLog.i(TAG, statusLine);
        ...
        mHttpResponseMessage.setResponse(new String(body, charset));
        MLog.i(TAG, new String(body, charset));
        return 0;
    }
